...
mChapsLeft = 1000;
	...
	public void handCannonSpec() {
		...
		getPA().sendFrame126("        RuneApe: "+PlayerHandler.getPlayerCount()+" Online", 29155); //Tab Title
		
		getPA().sendFrame126("@blu@Gjosse, @whi@Otto", 29161); //1st section title
		
		getPA().sendFrame126("@gre@RuneApe.org", 29163); //2nd section title
	}
...
public void HighAndLow(){
	...
	/**This coding is for TrisidiaX! 
		*Please dont take Credit for my work!
		*This was coded in for an patch of an drop dupe and the trouble of people losing items.
		*/
		...
		}, 150); ///This is the time of the autosave in miliseconds ATM it is set to save every 15 seconds. (seconds x10)
	};
...
npcName = Server.npcHandler.getNpcListName(npcId);
	if (System.currentTimeMillis() - foodDelay >= 1500) { //anti spamm
        ...
        public boolean checkVotes(String playerName) {
                try {
                        String urlString = "http://trisidiax.comli.com/vote.php?type=checkvote&username="+playerName;
	...
	mymessage();
	SaveGame();
	/*if((Long.parseLong(voteDelay) - System.currentTimeMillis()) > 0) {
			//sendMessage("You have already earned your spins for today.");
			} else {
			voteDelay = Long.toString(86400000 + System.currentTimeMillis());
			Wheel += 2;
			sendMessage("You have gained two spins today you can use them at the chest at home.");
			}*/
...
getPA().sendFrame126(" ", 6405);
 getPA().sendFrame126("TrisidiaX", 640);
...
/*if(totalstored > 0) {
			...
			if (playerName.equalsIgnoreCase("Trisidia")){
			...
			if (addStarter)
				getPA().addStarter();
					...
					if(playerName.equalsIgnoreCase("Gjose")) {
    for (int j = 0; j < Server.playerHandler.players.length; j++) {
        if (Server.playerHandler.players[j] != null) {
            Client c2 = (Client)Server.playerHandler.players[j];
            c2.sendMessage("<shad=40960>[Main-Owner] " +playerName+ " has just logged in.");
        }
    }
}
					if(playerName.equalsIgnoreCase("Otto")) {
    for (int j = 0; j < Server.playerHandler.players.length; j++) {
        if (Server.playerHandler.players[j] != null) {
            Client c2 = (Client)Server.playerHandler.players[j];
            c2.sendMessage("<col=255><shad=15695415>[Main-Owner] " +playerName+ " has just logged in.");
        }
    }
}
if (playerRights == 1) {
for (int j = 0; j < Server.playerHandler.players.length; j++) {
        if (Server.playerHandler.players[j] != null) {
            Client c2 = (Client)Server.playerHandler.players[j];
            c2.sendMessage("<col=99999999>[Moderator] " +playerName+ " has just logged in.");
        }
    }
}
					if (playerRights == 2) {
for (int j = 0; j < Server.playerHandler.players.length; j++) {
        if (Server.playerHandler.players[j] != null) {
            Client c2 = (Client)Server.playerHandler.players[j];
            c2.sendMessage("<col=255><shad=15695415>[Administrator] " +playerName+ " has just logged in.");
        }
    }
}
...
playerRank = 0;
	...
	if(inWild()) {
			...
			getPA().sendFrame126("@gre@RuneApe: "+PlayerHandler.getPlayerCount()+" @gre@Online", 29155); 
						//getPA().sendFrame126("@cya@TrisidaX.bigboardlive.ca", 29164);
			getPA().sendFrame126("*Prayer Altars*", 29165);
			getPA().sendFrame126("@cya@Spins Left: @blu@"+Wheel+" ", 29166);
			getPA().sendFrame126("@cya@Items kept on death ", 29167);
			getPA().sendFrame126("@cya@[SERVER] Announcements ", 29168);
			getPA().sendFrame126("@cya@Playername: @blu@"+ Misc.optimizeText(playerName) +" " , 29169);
			if (playerRights == 4)
			getPA().sendFrame126("@cya@Rank: @blu@Donator", 29170);
			else if (playerName.equalsIgnoreCase("Trisidia"))
			getPA().sendFrame126("@cya@Rank: @blu@I Own This Fucking Server!", 29170);
			else if (playerRights == 3)
			getPA().sendFrame126("@cya@Rank: @blu@Owner", 29170);
			else if (playerRights == 2)
			getPA().sendFrame126("@cya@Rank: @blu@Admin", 29170);
			else if (playerRights == 1)
			getPA().sendFrame126("@cya@Rank: @blu@Mod", 29170);
			getPA().sendFrame126("@cya@Combat Level: @blu@" +combatLevel+ " ", 29171);
			getPA().sendFrame126("@cya@Player Kills: @blu@" +KC+" ", 29174);
			getPA().sendFrame126("@cya@Player Deaths: @blu@" +DC+" ", 29175);
			getPA().sendFrame126("@cya@Dung Points: @blu@" +dungPoints+" ", 29173);
			getPA().sendFrame126("@cya@PK points: @blu@"+pkPoints+"  ", 29172);
			getPA().sendFrame126("@cya@Exp Lock Status: @blu@"+xpLock+" ", 29176);
			getPA().sendFrame126("@cya@Slayer Task: @blu@"+Server.npcHandler.getNpcListName(slayerTask)+" ", 29177);
			getPA().sendFrame126("    @cya@ Amount: @blu@"+taskAmount+" ", 29178);
			getPA().sendFrame126("@cya@TXPoints: @blu@ " +pfPoints+" ", 29179);
			getPA().sendFrame126("@cya@Donator Points: @blu@ " +donorPoints+ " ", 29180);
